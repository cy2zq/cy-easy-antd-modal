...
describe('Modal', () => {
  ...
  it('defaultOpen 正常工作', async () => {
    const onClean = vi.fn();
    render(
      <Modal title="cy-easy-antd-modal" defaultOpen onCancel={onClean}>
        I ❤️ antd
      </Modal>,
    );
  ...
  it('trigger 传入任意可添加 onClick 事件的组件都可以正常工作', async () => {
    const { container } = render(
      <Modal title="cy-easy-antd-modal" trigger={<div id="trigger"></div>}>
        I ❤️ antd
      </Modal>,
    );
  ...
  it('触发器的 onClick 事件会被劫持', async () => {
    const onClick = vi.fn();
    const { getByRole } = render(
      <Modal title="cy-easy-antd-modal" trigger={<Button onClick={onClick}>Click Me</Button>}>
        I ❤️ antd
      </Modal>,
    );
  ...
  it('触发器的 onClick 事件被 Model.onClick 代替, 并且需要手动调用打开', async () => {
    const onClick = vi.fn();
    const { getByRole } = render(
      <Modal title="cy-easy-antd-modal" onClick={onClick} trigger={<Button>Click Me</Button>}>
        I ❤️ antd
      </Modal>,
    );
  ...
  it('可以通过 actionRef 打开弹窗', async () => {
    ...
    render(
      <Modal title="cy-easy-antd-modal" actionRef={ref} trigger={<Button>Click Me</Button>}>
        I ❤️ antd
      </Modal>,
    );
  ...
  it('点击弹窗 footer 的 ok 按钮会触发 onOk， 并且关闭弹窗', async () => {
    const onOk = vi.fn();
    const { getByRole } = render(
      <Modal title="cy-easy-antd-modal" defaultOpen onOk={onOk} destroyOnClose>
        I ❤️ antd
      </Modal>,
    );
  ...
  describe('props.onCancel', () => {
    ...
    // cy-easy-antd-modal actionRef.close() 也可以可选的触发 onCancel
    describe('cy-easy-antd-modal', () => {
